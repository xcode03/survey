...
import (
	...
	"github.com/xcode03/survey/v2/core"
	"github.com/xcode03/survey/v2/terminal"
)
...
{{- else }}
	{{- "  "}}{{- color "cyan"}}{{- "\n"}}[Use arrows to move, space to select, type to filter{{- if and .Help (not .ShowHelp)}}, {{ .Config.HelpInput }} for more help{{end}}]{{color "reset"}}
  {{- "\n"}}
